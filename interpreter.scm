...
(define value
  (lambda (expr environment)
    (cond
      ...
      ((eq? (operator expr) '>=) (makeTuple (>= (getVal (value (operand1 expr) environment)) (getVal (value (operand2 expr) environment))) environment))
      ((eq? (operator expr) '<=) (makeTuple (<= (getVal (value (operand1 expr) environment)) (getVal (value (operand2 expr) environment))) environment))
      ((eq? (operator expr) '==) (makeTuple (eq? (getVal (value (operand1 expr) environment)) (getVal (value (operand2 expr) environment))) environment))
      ((eq? (operator expr) '!=) (makeTuple (not (eq? (getVal (value (operand1 expr) environment)) (getVal (value (operand2 expr) environment)))) environment))
      ((eq? (operator expr) '!) (makeTuple (not (getVal (value (operand1 expr) environment))) environment))
      ((eq? (operator expr) '&&) (makeTuple (and (getVal (value (operand1 expr) environment)) (getVal (value (operand2 expr) environment))) environment))
      ((eq? (operator expr) '||) (makeTuple (or (getVal (value (operand1 expr) environment)) (getVal (value (operand2 expr) environment))) environment))
      (else (error "wat")))))
